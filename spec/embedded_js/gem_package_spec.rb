...
RSpec.describe EmbeddedJs::GemPackage do
  describe '#with_path' do
    let(:io_mock) { instance_double('EmbeddedJs::Adapters::IO') }
    let(:subject) { EmbeddedJs::GemPackage.new(engine_name, io: io_mock, package_config: package_config) }

    context 'engine does specify specific files' do
      let(:engine_name) { 'package_a' }
      let(:package_config) { instance_double('EmbeddedJs::PackageConfig') }
      let(:expected_tmp_path) { "/tmp/EmbeddedJs/#{engine_name}" }
